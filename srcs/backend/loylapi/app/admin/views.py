...
from app.models import (
    User, Business, Token, Wallet, Balance,
    PointTx, SwapTx, VoucherTemplate, VoucherNFT,
    BusinessReview, PromotionCampaign, TokenPool,
    CeleryTaskLog, Admin
)
...
class UserAdmin(BaseAdminView, model=User):
    column_list = [User.id, User.privy_id, User.email, User.phone,
                   User.created_at]
...
class BusinessAdmin(BaseAdminView, model=Business):
    column_list = [
        ...
        Business.owner_email, Business.rate_loyl,
        Business.created_at
    ]
...
class TokenAdmin(BaseAdminView, model=Token):
    column_list = [
        Token.id, Token.symbol, Token.mint,
        Token.business_id, Token.rate_loyl,
        Token.total_supply, Token.decimals
    ]
...
class BalanceAdmin(BaseAdminView, model=Balance):
    column_list = [
        Balance.id, Balance.wallet_id,
        Balance.token_id, Balance.amount,
        Balance.updated_at
    ]
...
class PointTxAdmin(BaseAdminView, model=PointTx):
    column_list = [
        PointTx.id, PointTx.wallet_id, PointTx.token_id,
        PointTx.tx_type, PointTx.amount, PointTx.sol_sig,
        PointTx.created_at
    ]
...
class SwapTxAdmin(BaseAdminView, model=SwapTx):
    column_list = [
        SwapTx.id, SwapTx.user_id, SwapTx.from_token_id,
        SwapTx.to_token_id, SwapTx.from_amount, SwapTx.to_amount,
        SwapTx.status, SwapTx.created_at
    ]
...
class CeleryTaskLogAdmin(BaseAdminView, model=CeleryTaskLog):
    column_list = [
        CeleryTaskLog.id, CeleryTaskLog.task_id,
        CeleryTaskLog.queue, CeleryTaskLog.status,
...
class AdminAdmin(BaseAdminView, model=Admin):
    """Admin view"""
    column_list = [
        Admin.id, Admin.email, Admin.is_totp_enabled,
        Admin.is_active, Admin.last_login, Admin.created_at
    ]
    ...
    form_excluded_columns = [Admin.password_hash, Admin.totp_secret,
                             Admin.last_login]
...
class TokenPoolAdmin(BaseAdminView, model=TokenPool):
    column_list = [
        TokenPool.id, TokenPool.token_id,
        TokenPool.balance_token, TokenPool.balance_loyl,
        TokenPool.provider
    ]
    can_create = False
    can_edit = False
...
class VoucherTemplateAdmin(BaseAdminView, model=VoucherTemplate):
    column_list = [
        VoucherTemplate.id, VoucherTemplate.business_id,
        VoucherTemplate.name, VoucherTemplate.price_points,
        VoucherTemplate.supply, VoucherTemplate.expires_at
    ]
...
class BusinessReviewAdmin(BaseAdminView, model=BusinessReview):
    column_list = [
        BusinessReview.id, BusinessReview.business_id,
        BusinessReview.user_id, BusinessReview.score,
